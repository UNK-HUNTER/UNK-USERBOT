...
def main():
    print("T E A M    U N K   U S E R B O T   ! !")
    print("Hello!! Welcome to UNK Session Generator\n")
...
def generate_hellbot_session():
    print("!!! UNK USERBOT SESSION !!!")
    print("One session for all UNK UserBot's Project.")
    api_id = int(input("\nEnter APP ID here: "))
    api_hash = input("\nEnter API_HASH here: ")
    with PClient(name="helluser", api_id=api_id, api_hash=api_hash, in_memory=True) as hell:
        print("\nYour UNK USERBOT SESSION is saved in your telegram saved messages.")
        _session = hell.export_session_string()
        hell_session = hellbot_session(_session)
        hell.send_message(
            "me",
            f"#UNK_USERBOT_SESSION \n\n`{hell_session}`",
        )
...
def generate_pyro_session():
    ...
    with PClient(name="helluser", api_id=APP_ID, api_hash=API_HASH, in_memory=True) as hellbot:
        print("\nYour UNK UserBot Session Is sent in your Telegram Saved Messages.")
        hellbot.send_message(
            "me",
            f"#UNK_UserBOT #PYROGRAM\n\n`{hellbot.export_session_string()}`",
        )


def generate_telethon_session():
    print("\nTelethon Session For UNK UserBot!")
    APP_ID = int(input("\nEnter APP ID here: "))
    API_HASH = input("\nEnter API HASH here: ")
...
def hellbot_session(session):
    ...
    error_msg = "Error in generating session! Report it in UNK Chats"
